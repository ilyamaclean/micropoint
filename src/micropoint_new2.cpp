...
// than surface-layer theory allows for. Raupach (1994) Eq. 5 sets the size of
// that departure at the canopy top as ln(c_w) - 1 + 1/c_w with his c_w = 2.
...
// Be (sqrt(0.003+0.1*pai)) as a stand-in for u*/Uh and the fixed
...
double roughlengthCpp2(double h, double pai, double d)
{
    ...
    // The expression describes roughness generated by the foliage and carries no
    // term for the ground beneath it, so for short or open vegetation it can
    // return less than the soil's own roughness -- a canopy smoother than the
    // surface it grows on, which the wind never sees. Below this the ground
    // governs, and the same value is used for bare soil elsewhere in the model.
    //
    // For vegetation of a few millimetres this floor and the cap above it cannot
    // both hold, and the floor is applied second so it wins: a surface whose soil
    // is rougher than its plants are tall is a bare-ground surface. Canopies that
    // short are turned away before reaching here.
    if (zm < Z0_GROUND) zm = Z0_GROUND;
    return zm;
}
    ...
    double zi = 1000.0, double beta = 1.0)
{
    ...
    // Canopy top lies inside the roughness sublayer, where wind exceeds what a
    // surface-layer profile gives. Taking it from such a profile overstates the
    // wind driving leaf exchange, and understates in the same proportion the drag
    // the canopy exerts on the air above.
    double lnterm = std::log((hgt - d) / zm) - PSI_RSL + psi_m;
    if (lnterm < LNTERM_MIN) lnterm = LNTERM_MIN;
    double uh = (uf / ka) * lnterm;
